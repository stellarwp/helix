...
class Usage_Error extends \Exception {
	...
	public static function because_where_or_should_only_be_used_with_methods_that_add_where_clauses( array $method, $object ) {
		...
		return new self( "You are trying to build a WHERE OR clause using a method ({$class}::{$method}) that does not call the Query_Filters::where method directly; call `where_clause` directly or call methods that call it." );
	}
