...
abstract class Repository implements Repository_Interface {
	...
	/**
	 * @var Repository\Formatter_Interface
	 */
	protected $formatter;
	 ...
	 *
	 * @return mixed|null
	 * @throws Repository\Usage_Error If trying to access a non defined property.
	 */
	public function __get( $name ) {
		if ( ! property_exists( $this, $name ) ) {
			throw Repository\Usage_Error::because_property_is_not_defined( $name, $this );
		}
	 ...
	 * @param mixed  $value
	 *
	 * @throws Repository\Usage_Error As properties have to be set extending
	 * the class, using setter methods or via constructor injection
	 */
	public function __set( $name, $value ) {
		throw Repository\Usage_Error::because_properties_should_be_set_correctly( $name, $this );
	}
	 ...
	 * @return mixed
	 *
	 * @throws Repository\Usage_Error If the required filter is not defined by the class.
	 * @throws Repository\Void_Query_Exception To signal the query would yield no results.
	 */
	protected function modify_query( $key, $call_args ) {
		if ( ! $this->schema_has_modifier_for( $key ) ) {
			if ( $this->has_default_modifier( $key ) ) {
				...
				throw Repository\Usage_Error::because_the_read_filter_is_not_defined( $key, $this );
			}
		} else {
	 ...
	 *
	 * @return Repository\Read_Interface|Repository\Update_Interface
	 */
	...
	public function get_ids() {
		...
		} catch ( Repository\Void_Query_Exception $e ) {
			/*
	 ...
	 *
	 * @throws Repository\Usage_Error
	 *
	 * @return $this
	 */
	public function set( $key, $value ) {
		if ( ! is_string( $key ) ) {
			throw Repository\Usage_Error::because_update_key_should_be_a_string( $this );
		}
	 ...
	 * {@inheritdoc}
	 */
	public function set_formatter( Repository\Formatter_Interface $formatter ) {
	...
	public function where_meta_related_by( $meta_keys, $compare, $field = null, $values = null ) {
		...
		if ( ! in_array( $compare, [ 'EXISTS', 'NOT EXISTS' ], true ) ) {
			if ( empty( $field ) || empty( $values ) ) {
				throw Repository\Usage_Error::because_this_comparison_operator_requires_fields_and_values( $meta_keys, $compare, $this );
			}
		}
	 ...
	 *
	 * @return $this
	 * @throws Repository\Usage_Error If one of the callback methods does
	 *                                        not add any WHERE clause.
	 *
	 * @see Repository::where_clause()
	 * @see Repository\Query_Filters::where()
	 */
	public function where_or( $callbacks ) {
		...
		foreach ( $all_callbacks as $c ) {
			...
			if ( $buffered_count === count( $this->filter_query->get_buffered_where_clauses() ) ) {
				throw Repository\Usage_Error::because_where_or_should_only_be_used_with_methods_that_add_where_clauses( $c, $this );
			}
	 ...
	 *
	 * @return array
	 * @throws Repository\Usage_Error If a filter is called with wrong arguments.
	 */
	 ...
	 *
	 * @return array|null
	 * @throws Repository\Usage_Error If trying to compare multiple values with a single
	 *                                        comparison operator.
	 */
	protected function build_meta_query( $meta_key, $meta_value = 'value', $compare = '=', $type_or_format = null ) {
		...
		if ( count( $meta_keys ) === 1 ) {
			...
			if ( 0 === strpos( $type_or_format, '%' ) ) {
				throw Repository\Usage_Error::because_the_type_is_a_wpdb_prepare_format( $meta_key, $type_or_format, $this );
			}
		...
		if ( null === $type_or_format ) {
			...
			throw Repository\Usage_Error::because_the_format_is_not_a_wpdb_prepare_one( $meta_key, $type_or_format, $this );
		}
	...
	protected function validate_operator_and_values( $compare, $meta_key, $meta_value ) {
		if ( is_array( $meta_value ) && ! in_array( $compare, self::$multi_value_keys, true ) ) {
			throw Repository\Usage_Error::because_single_value_comparisons_should_be_used_with_one_value(
				$meta_key,
				$meta_value,
				$compare,
				$this
	 ...
	 *
	 * @throws Repository\Usage_Error If the relation is not a valid one.
	 */
	protected function validate_relation( $relation ) {
		if ( ! in_array( $relation, [ 'OR', 'AND' ], true ) ) {
			throw Repository\Usage_Error::because_this_relation_is_not_valid( $relation );
		}
	}
	 ...
	 *
	 * @throws Repository\Usage_Error If the relations are not valid or another WHERE building issue happens.
	 */
