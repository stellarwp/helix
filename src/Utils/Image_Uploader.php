...
class Image_Uploader {
	...
	/**
	 * Original URL meta key.
	 * 
	 * @since 1.0.0
	 *
	 * @var string
	 */
	protected static $original_url_meta_key = '_helix_importer_original_url';

	/**
	 * Image_Uploader constructor.
	 * 
	 * @since 1.0.0
	 *
	...
	protected function maybe_retry_upload( $id ) {
		if ( $id ) {
			Log::log( Log::DEBUG, __CLASS__, [ 'message' => "ID: {$id} is already a valid one." ] );
	...
	protected function upload_file( $file_url ) {
		...
		update_post_meta( $attachment_id, static::$original_url_meta_key, $file_url );
	...
	/**
	 * Gets the attachement ID from a given URL.
	 * 
	 * @since 1.0.0
	 *
	 * @param string $featured_image The URL of the image.
	 */
	...
	protected function maybe_init_attachment_original_urls_cache() {
		if ( false === self::$original_urls_cache ) {
			...
			$original_urls = $wpdb->get_results( "
				...
				AND pm.meta_key = '" . static::$original_url_meta_key . "'
			" );
