...
class Timezones {
	...
	public static function abbr( $date, $timezone_string ) {
		try {
			...
			$date_time = $date instanceof DateTime
				...
				? $date
				: Date_Utils::build_date_object( $date, $timezone_object );
	...
	public static function to_utc( $datetime, $tzstring, $format = null ) {
		...
		if ( $new_datetime ) {
			...
			$format = ! empty( $format ) ? $format : Date_Utils::DBDATETIMEFORMAT;
	...
	public static function to_tz( $datetime, $tzstring ) {
		...
		if ( $new_datetime && $new_datetime->setTimezone( $local ) ) {
			return $new_datetime->format( Date_Utils::DBDATETIMEFORMAT );
		}
	...
	public static function apply_offset( $datetime, $offset, $invert = false ) {
		...
		if ( $offset_datetime && $offset_datetime->modify( $offset ) ) {
			return $offset_datetime->format( Date_Utils::DBDATETIMEFORMAT );
		}
	...
	public static function maybe_get_tz_name( $timezone ) {
		...
		$timezone = timezone_name_from_abbr( '', $offset * 3600, true );
		...
		if ( false === $timezone ) {
			$timezone = timezone_name_from_abbr( '', (int) $offset * 3600, true );
		}

		// lastly if that didn't work, round the gmt_offset down and maybe that TZ doesn't do daylight savings
		if ( false === $timezone ) {
			$timezone = timezone_name_from_abbr( '', (int) $offset * 3600, false );
		}
	...
	public static function adjust_timestamp( $unix_timestamp, $tzstring ) {
		try {
			...
			$datetime = date_create_from_format( 'U', $unix_timestamp )->format( Date_Utils::DBDATETIMEFORMAT );
	...
	public static function build_timezone_object( $timezone = null ) {
		...
		if ( is_string( $timezone ) && isset( $cache[ __METHOD__ . $timezone ] ) && $cached = $cache[ __METHOD__ . $timezone ] ) {
			return clone $cached;
		}
