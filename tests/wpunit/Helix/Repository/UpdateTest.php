...
use StellarWP\Helix\Repository as Update_Repository;
use StellarWP\Helix\Repository\Decorator as Decorator;
...
class UpdateTest extends \Codeception\TestCase\WPTestCase {
	...
	public function setUp() {
		...
		$this->class = new class extends Update_Repository {
	...
	public function should_allow_setting_featured_image_with_url_using_method() {
		...
		add_filter( 'stellar_image_uploader_local_urls', '__return_true' );
		$this->repository()->where( 'post__in', $ids )->set_featured_image( $image )->save();
		remove_filter( 'stellar_image_uploader_local_urls', '__return_true' );
	...
	public function should_throw_if_trying_to_set_a_blocked_field( $field ) {
		...
		$this->expectException( \StellarWP\Helix\Repository\Usage_Error::class );
	...
	public function should_update_posts_in_background_if_over_threshold() {
		add_filter( 'stellar_repository_books_update_background_activated', '__return_true' );
		add_filter( 'stellar_repository_books_update_background_threshold', function () {
			return 1;
		} );
		$ids     = $this->factory()->post->create_many( 2, [ 'post_type' => 'book' ] );
	...
	public function should_allow_filtering_update_payloads_from_decorator() {
		...
		$decorator = new class( $this->repository() ) extends Decorator {
			public function __construct( \StellarWP\Helix\Repository\Interface $decorated ) {
				$this->decorated = $decorated;
				$filter_name     = $decorated->get_filter_name();
				add_filter( "stellar_repository_{$filter_name}_update_postarr", [
					$this,
